...
export class PayNowDecoder {
  ...
  private static extractPayNowData(fields: DecodedField[]): PayNowData {
    const merchantInfoField = fields.find(
      (f) => f.id === this.MERCHANT_ACCOUNT_INFO
    );
    const additionalDataField = fields.find(
      (f) => f.id === this.ADDITIONAL_DATA
    );
    const transactionAmount = this.extractFieldValue(
      fields,
      this.TRANSACTION_AMOUNT
    );

    return {
      payloadFormat: this.extractFieldValue(
        fields,
        this.PAYLOAD_FORMAT_INDICATOR
      ),
      pointOfInitiation: this.extractFieldValue(
        fields,
        this.POINT_OF_INITIATION_METHOD
      ),
      merchantAccountInfo: this.parseMerchantAccountInfo(
        merchantInfoField?.nestedFields || []
      ),
      merchantCategoryCode: this.extractFieldValue(
        fields,
      ...
      additionalData: this.parseAdditionalData(
        additionalDataField?.nestedFields || []
      ),
  ...
  private static parseMerchantAccountInfo(
    nestedFields: DecodedField[]
  ): PayNowData["merchantAccountInfo"] {
    console.log(
      "🚀 ~ PayNowDecoder ~ parseMerchantAccountInfo ~ nestedFields:",
      nestedFields
    );
    const identifier = this.extractFieldValue(
      nestedFields,
      this.MERCHANT_IDENTIFIER
    );
    const rawProxyType = this.extractFieldValue(
      nestedFields,
      this.MERCHANT_PROXY_TYPE
    );
    ...
    const proxyType =
      ...
      undefined;
    const amountEditableStr = this.extractFieldValue(
      nestedFields,
      this.MERCHANT_AMOUNT_EDITABLE
    );

    return {
      identifier: identifier,
      proxyType: proxyType,
      proxyValue: this.extractFieldValue(
        nestedFields,
        this.MERCHANT_PROXY_VALUE
      ),
      amountEditable: amountEditableStr === "1",
      expiryDate: this.extractFieldValue(
        nestedFields,
        this.MERCHANT_EXPIRY_DATE
      ),
    };
  }
  ...
  private static parseAdditionalData(
    nestedFields: DecodedField[]
  ): PayNowData["additionalData"] | undefined {
    const referenceNumber = this.extractFieldValue(
      nestedFields,
